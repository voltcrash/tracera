...
* **Ground Zero** — Tracera's key differentiator: trace and surface the *original source* of a given news item, not just corroborating sources.
...
* **Evidence-quality confidence** — separate from the claim verdict itself, disclose how strong/weak the *evidence* behind that verdict is (e.g., a claim may be accurate but only backed by thin evidence because the news is very recent).
