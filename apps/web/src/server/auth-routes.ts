...
/** The Better Auth dashboard is the only cross-origin caller of these routes. */
const ALLOWED_BROWSER_ORIGINS = new Set(["https://dash.better-auth.com"]);
...
authRoutes.all("/*", async (context) => {
  if (context.req.method === "OPTIONS") {
    return withDashboardCors(new Response(null, { status: 204 }), context.req.raw);
  }
  ...
  return withDashboardCors(publicAuthResponse(response, context.req.raw), context.req.raw);
});
...
function withDashboardCors(response: Response, request: Request) {
  ...
  response.headers.set("access-control-expose-headers", "Set-Auth-Token, X-Request-ID");
