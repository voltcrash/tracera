...
export function GroundZeroCard({ trace }: { trace: GroundZeroTrace }) {
  ...
  return (
    <section className="ground-zero-result landing-view-reveal mt-5 overflow-hidden rounded-[2rem] border border-emerald-800/12 bg-[#d9f5eb] p-6 shadow-[0_24px_60px_-46px_rgba(16,34,31,.58)] sm:p-8">
      <div className="grid gap-8 lg:grid-cols-[.75fr_1.25fr] lg:items-center">
        <div>
          ...
          <p className="mt-4 max-w-md text-sm leading-6 text-emerald-950/58">
            {trace.earliestSource
        ...
        <div className="rounded-2xl border border-emerald-950/8 bg-white/68 p-4 sm:p-5">
            ...
            {trace.signals.slice(0, 3).map((signal, index) => (
              <div key={`${signal}-${index}`} className="ground-zero-step">
                ...
                <div className="min-w-0">
                  <p className="text-[9px] font-black uppercase tracking-[.12em] text-emerald-900/40">
                    Signal {String(index + 1).padStart(2, "0")}
                  </p>
                  <p className="mt-1 text-xs font-bold leading-5 text-emerald-950/68">
                    {signal}
                  </p>
                </div>
              </div>
                  ...
                  trace.earliestSource.url ? (
                    ...
                    >
                      ...
                      {trace.earliestSource.publisher && (
                        <small className="mt-1 block text-[10px] font-semibold text-emerald-950/45">
                          {trace.earliestSource.publisher}
                        </small>
                      )}
                    </a>
