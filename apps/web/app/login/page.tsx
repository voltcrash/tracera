  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await fetch(
        `${apiUrl}/auth/${mode === "login" ? "login" : "signup"}`,
        {
          method: "POST",
          credentials: "include",
    ...
    <main className="grid min-h-screen bg-[#f7faf9] lg:grid-cols-2">
      <section className="hidden bg-emerald-950 p-12 text-white lg:flex lg:flex-col lg:justify-between">
        ...
        <p className="text-sm text-white/50">
          Email and password only. No social sign-in or two-factor setup.
        </p>
      </section>
      <section className="flex items-center justify-center p-6 sm:p-10">
        <div className="w-full max-w-md">
          ...
          <p className="mt-12 text-[10px] font-black tracking-[.2em] text-emerald-700">
            YOUR TRACERA ACCOUNT
          </p>
          <h1 className="mt-3 text-3xl font-extrabold tracking-[-.03em]">
          ...
          <form className="mt-8 space-y-4" onSubmit={submit}>
            <label className="block text-sm font-semibold text-slate-700">
              ...
              <input
                ...
                className="mt-2 w-full rounded-xl border border-slate-300 bg-white px-4 py-3 outline-none focus:border-emerald-600 focus:ring-4 focus:ring-emerald-50"
              />
            </label>
            <label className="block text-sm font-semibold text-slate-700">
              Password
            ...
            <button
              disabled={submitting}
              className="w-full rounded-xl bg-emerald-800 py-3 text-sm font-bold text-white transition hover:bg-emerald-950 disabled:cursor-not-allowed disabled:bg-slate-400"
            >
              {submitting
                ? "Please wait…"
                : signingUp
