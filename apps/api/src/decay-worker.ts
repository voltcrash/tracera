...
new Worker("tracera-decay", async (job) => {
  ...
  for (const check of await dueChecks()) {
    try {
      const response = await fetch(`${process.env.INTERNAL_API_URL ?? "http://127.0.0.1:3001"}/analyze`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(check.input_type === "link" ? { url: check.source_url } : { text: check.raw_input }) });
