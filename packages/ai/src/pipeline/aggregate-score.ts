import type { ClaimVerdict, ScoreDimension, TraceraScore } from "./types.js";

export function aggregateScore(claims: ClaimVerdict[]): TraceraScore {
  ...
  const sourceCorroboration = average(
    claims.map((claim) => Math.min((claim.supportingSources.length + claim.contradictingSources.length) / 3, 1)),
  );
  const evidenceQuality = average(claims.map((claim) => claim.evidenceQuality));
  const sourceReputation = average(claims.flatMap((claim) => [...claim.supportingSources, ...claim.contradictingSources]).map((source) => source.credibility ?? 0.5));
  const framingManipulation = average(
    claims.map((claim) => framingScore(claim.claim.claimText, claim.claim.claimType)),
  );
  const newestEvidenceAt = claims
    .flatMap((claim) => [...claim.supportingSources, ...claim.contradictingSources])
    .map((source) => source.publishedAt)
    .filter((date): date is string => Boolean(date))
    .sort((left, right) => Date.parse(right) - Date.parse(left))[0] ?? null;
  const overall = average([factualAccuracy, sourceCorroboration, framingManipulation, evidenceQuality, sourceReputation]);
...
function verdictValue(verdict: ClaimVerdict["verdict"]) {
  return { supported: 1, mixed: 0.55, misleading: 0.35, contradicted: 0, unverified: 0.5 }[verdict];
}
...
function framingScore(text: string, type: ClaimVerdict["claim"]["claimType"]) {
  ...
  const typePenalty = type === "framing" ? 0.45 : type === "opinion" ? 0.2 : 0;

  return Math.max(0, 1 - typePenalty - matches * 0.15);
}
...
function weightedAverage<T>(items: T[], getValue: (item: T) => number, getWeight: (item: T) => number) {
...
function dimension(score: number): ScoreDimension {
  return { score: toPercent(score), label: score >= 0.75 ? "strong" : score >= 0.45 ? "moderate" : "weak" };
}
