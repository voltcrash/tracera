...
function estimateEvidenceQuality(sources: EvidenceSource[]) {
  ...
  const credibility = sources.reduce((sum, source) => sum + (source.credibility ?? 0.5), 0) / sources.length;
  const relevance = sources.reduce((sum, source) => sum + (source.similarity ?? 0.7), 0) / sources.length;
  return Number((0.25 * corroboration + 0.2 * recency + 0.2 * credibility + 0.35 * relevance).toFixed(4));
}
