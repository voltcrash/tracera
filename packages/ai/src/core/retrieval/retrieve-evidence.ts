...
export const retrieveEvidenceV2: RetrieveEvidenceV2 = createRetrieveEvidenceV2();

export function createRetrieveEvidenceV2(options: RetrievalOptions = {}): RetrieveEvidenceV2 {
...
async function retrieve(
  ...
  const data: RetrieveEvidenceV2Data = {
  ...
  async function retrieveCorpus(claim: ClaimV2) {
  ...
  async function canceledResult(): Promise<StageResult<RetrieveEvidenceV2Data>> {
    await audit(
      "cancellation",
  ...
  function finish(
    status: StageResult<RetrieveEvidenceV2Data>["status"],
    data: RetrieveEvidenceV2Data | null,
    finalIssues: CoreIssue[],
  ): StageResult<RetrieveEvidenceV2Data> {
...
function eligibleClaims(input: RetrieveEvidenceV2Input, issues: CoreIssue[]) {
  const seen = new Set<string>();
  const result: ClaimV2[] = [];
...
function temporalScopeMatches(claim: ClaimV2, candidate: ClaimV2["time"]["interval"]) {
