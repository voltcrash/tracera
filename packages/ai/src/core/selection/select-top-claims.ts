...
interface WorkingClaim {
  claim: ClaimV2;
  originalCoverageDisposition: ClaimV2["coverageDisposition"];
  canonical: boolean;
...
const CHECKABILITY_RANK: Record<ClaimV2["checkability"], number> = {
  checkable: 3,
  needs_context: 2,
  unanswerable: 1,
  not_checkable: 0,
...
export function selectTopClaims(input: FocusedSelectionInput): FocusedSelectionResult {
  ...
  const normalizedClaims = parsedClaims.map((claim) => {
    ...
    return claimSchema.parse({ ...claim, duplicateOfClaimId }) satisfies ClaimV2;
  });
  ...
  const selected = eligible.slice(0, CORE_V2_FOCUSED_MAX_SELECTED_CLAIMS);
  ...
  const finalClaims = normalizedClaims.map((claim) => {
    ...
    if (
      ...
      return claimSchema.parse({ ...claim, coverageDisposition: "deferred" }) satisfies ClaimV2;
    }
    return claim;
  });
  ...
  const selection = focusedSelectionSchema.parse({
    policyVersion: CORE_V2_FOCUSED_POLICY_VERSION,
    selectionVersion: CORE_V2_FOCUSED_SELECTION_VERSION,
    maxSelectedClaims: CORE_V2_FOCUSED_MAX_SELECTED_CLAIMS,
    inputSnapshotHash:
    ...
    excludedClaimIds,
    shortfallReason:
      selected.length < CORE_V2_FOCUSED_MAX_SELECTED_CLAIMS
  ...
  if (selectionDeferred.length > 0) {
    issues.push({
      ...
      severity: "warning",
      message: `${selectionDeferred.length} canonical factual claim(s) were inventoried but deferred outside the focused limit of ${CORE_V2_FOCUSED_MAX_SELECTED_CLAIMS}.`,
      claimId: null,
      snapshotId: null,
      url: null,
    });
...
function compareDocumentOrder(left: ClaimV2, right: ClaimV2, documentOrder: Map<string, number>) {
  return (
    compareNullable(
...
function shortfallReason(selected: number, eligible: number, inventoryStatus: StageStatus) {
  if (inventoryStatus !== "complete") {
    return `Only ${selected} of ${CORE_V2_FOCUSED_MAX_SELECTED_CLAIMS} focused slots were selected from a ${inventoryStatus} inventory; incomplete or omitted input remains explicit.`;
  }
  if (eligible === 0) {
...
function propositionScopeKey(claim: ClaimV2) {
...
function firstSpanStart(claim: ClaimV2) {
...
function isMeasurableEvent(claim: ClaimV2) {
  return (
