...
import { createRetrieveEvidence } from "../retrieval/index";
import type {
  AdjudicateClaims,
  AdjudicateClaimsData,
  AuditEvent,
  GenerationRequest,
  PortUsage,
  RunEnvironment,
...
export const adjudicateClaims: AdjudicateClaims = createAdjudicateClaims();

export function createAdjudicateClaims(options: AdjudicationOptions = {}): AdjudicateClaims {
...
async function adjudicate(
  input: Parameters<AdjudicateClaims>[0],
  ...
  async function targetedRound(
    ...
    if (admitted.length > 0) {
      ...
      const assess =
        ...
        createAssessEvidence({ maxGenerationRequests });
  ...
  function targetedFeedback(claim: Claim, assessments: EvidenceAssessment[]): SufficiencyFeedback {
  ...
  async function integrityFailure(claim: Claim, reasons: string[]): Promise<Decision> {
  ...
  function exhausted(claim: Claim) {
    warn(
      "budget_exhausted",
  ...
  async function loadSnapshots(claims: Claim[], assessments: EvidenceAssessment[]) {
  ...
  async function finish(
    status: StageResult<AdjudicateClaimsData>["status"],
    data: AdjudicateClaimsData | null,
  ): Promise<StageResult<AdjudicateClaimsData>> {
