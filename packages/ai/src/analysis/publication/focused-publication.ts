...
export const FOCUSED_PUBLICATION_POLICY: FocusedPublicationPolicy =
  focusedPublicationPolicySchema.parse({
    ...
    mode: "evidence_gated",
    scoreFormulaVersion: ANALYSIS_FOCUSED_SCORE_FORMULA_VERSION,
    calibration: {
      status: "not_used",
      probability: null,
      reason: ANALYSIS_FOCUSED_NON_CALIBRATION_REASON,
    },
  });
 ...
 */
export function publishFocusedDecisions(input: FocusedPublicationInput): FocusedPublicationData & {
...
export function createFocusedPublication(): FocusedPublication {
  return async (input, environment) => {
    ...
    try {
      ...
      const data: FocusedPublicationData = {
    ...
    function finish(
      status: StageResult<FocusedPublicationData>["status"],
      data: FocusedPublicationData | null,
      issues: AnalysisIssue[],
    ): StageResult<FocusedPublicationData> {
      return {
        status,
        data,
        issues,
