...
import type { ScoreReport, ScoreReportData } from "../types";

export const scoreReport: ScoreReport = (input) => {
  const metrics = {
    startedAt: input.at,
...
function scoreNullReasons(input: {
  ...
  if (
    ...
    input.resolutionCoverage < ANALYSIS_RESOLUTION_COVERAGE_THRESHOLD
  )
...
function extractionCoverage(coverage: Parameters<ScoreReport>[0]["coverage"]) {
...
function evidenceSummary(assessments: Parameters<ScoreReport>[0]["assessments"]) {
...
function originSummary(graphs: Parameters<ScoreReport>[0]["graphs"]) {
  return {
...
function requireClaim(claimById: Map<string, Claim>, id: string) {
  const claim = claimById.get(id);
  if (claim === undefined) throw new Error(`Unknown claim reference: ${id}.`);
  return claim;
}

function requireClaimLink(claimById: Map<string, Claim>, claim: Claim, id: string) {
...
function validateDecisionReferences(
  decisions: Parameters<ScoreReport>[0]["decisions"],
  assessments: Parameters<ScoreReport>[0]["assessments"],
) {
...
export type ScoreResult = StageResult<ScoreReportData>;
