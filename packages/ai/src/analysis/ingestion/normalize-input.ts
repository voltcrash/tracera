import type { AnalysisIssue, StageMetrics } from "@repo/contracts/analysis";
import type { NormalizeInput } from "../types";
import { supportsImageAcquisition } from "./types";

export const normalizeInput: NormalizeInput = async ({ input }, environment) => {
...
async function finishAudit(
  environment: Parameters<NormalizeInput>[1],
  status: string,
...
function cancellationIssue(): AnalysisIssue {
  return {
...
function stageMetrics(
  clock: Parameters<NormalizeInput>[1]["ports"]["clock"],
  startedAt: string,
  startedMs: number,
