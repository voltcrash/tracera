import type { AnalysisIssue, DocumentSnapshot, StageMetrics } from "@repo/contracts/analysis";
import type { AuditEvent, ExtractClaims, RunEnvironment } from "../types";
...
const UNUSABLE_EXTRACTION = new Map<DocumentSnapshot["extractionStatus"], AnalysisIssue["code"]>([
  ["content_unavailable", "content_unavailable"],
  ["blocked", "blocked_page"],
  ["unsupported_format", "unsupported_format"],
]);

export function createExtractClaims(options: ClaimExtractionOptions = {}): ExtractClaims {
  ...
  return async ({ snapshots, primarySnapshotId }, environment) => {
    ...
    const withoutData = async (status: "unavailable" | "failed", stageIssues: AnalysisIssue[]) => {
      await finish(status);
...
export const extractClaims: ExtractClaims = createExtractClaims();
...
function unusableReason(snapshot: DocumentSnapshot): AnalysisIssue | null {
