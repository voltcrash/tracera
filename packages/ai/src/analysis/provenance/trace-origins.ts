...
import type { RunEnvironment, TraceOrigins, TraceOriginsData } from "../types";
import {
  archiveAssertion,
  classifyNode,
...
export const traceOrigins: TraceOrigins = createTraceOrigins();

export function createTraceOrigins(options: ProvenanceOptions = {}): TraceOrigins {
  return async (input, environment) => trace(input, environment, options);
}

async function trace(
  input: Parameters<TraceOrigins>[0],
  ...
  async function traceClaim(claim: Claim): Promise<ProvenanceGraph> {
    ...
    async function assessSnapshot(scopedClaim: Claim, snapshot: DocumentSnapshot) {
      if (budgetBlocked(1)) {
        incomplete = true;
        issues.push(
          issue(
    ...
    async function lookupArchive(scopedClaim: Claim, snapshot: DocumentSnapshot) {
  ...
  async function canceledResult(): Promise<StageResult<TraceOriginsData>> {
  ...
  function finish(
    status: StageResult<TraceOriginsData>["status"],
    data: TraceOriginsData | null,
  ): StageResult<TraceOriginsData> {
    return {
      status,
      data,
      issues,
    ...
    } as StageResult<TraceOriginsData>;
  }
...
function uniqueSnapshots(snapshots: DocumentSnapshot[], issues: AnalysisIssue[]) {
