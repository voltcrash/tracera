...
import type { AssessEvidence, AssessEvidenceData, AuditEvent, RunEnvironment } from "../types";
...
export const assessEvidence: AssessEvidence = createAssessEvidence();

export function createAssessEvidence(options: EvidenceAssessmentOptions = {}): AssessEvidence {
  return async (input, environment) => assess(input, environment, options);
}

async function assess(
  input: Parameters<AssessEvidence>[0],
  environment: RunEnvironment,
  options: EvidenceAssessmentOptions,
): Promise<ReturnType<AssessEvidence> extends Promise<infer T> ? T : never> {
