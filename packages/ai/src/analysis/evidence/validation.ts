...
import type { Claim, DocumentSnapshot, EvidenceAssessment } from "@repo/contracts/analysis";
import { evidenceAssessmentSchema } from "@repo/contracts/analysis";
...
function entityCheck(claim: Claim, quote: string, relation: RawAssessment["relation"]) {
...
function temporalCheck(claim: Claim, quote: string, relation: RawAssessment["relation"]) {
  if (["irrelevant", "insufficient", "context"].includes(relation))
    return { pass: true, detail: "No factual entailment is claimed." };
...
function jurisdictionCheck(claim: Claim, quote: string) {
  if (!claim.place)
...
function attributionCheck(claim: Claim, quote: string, relation: RawAssessment["relation"]) {
  if (["irrelevant", "insufficient", "context"].includes(relation))
    return { pass: true, detail: "No factual entailment is claimed." };
  const quoteText = normalized(quote);
...
function negationCheck(claim: Claim, quote: string, relation: RawAssessment["relation"]) {
...
function numericChecks(claim: Claim, quote: string, relation: RawAssessment["relation"]) {
