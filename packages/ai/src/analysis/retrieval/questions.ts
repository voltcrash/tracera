import type { Claim, QueryIntent, SufficiencyFeedback } from "@repo/contracts/analysis";
import type { RetrievalQuestion } from "./types";

const quote = (value: string) => `"${value.replaceAll('"', "").trim()}"`;

export function buildPropositionKey(claim: Claim): string {
...
function scopedTerms(claim: Claim): string {
...
function knownInterval(claim: Claim) {
